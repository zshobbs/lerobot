...
from lerobot.robots.alohamini.config_lekiwi_uni import LeKiwiUniClientConfig
from lerobot.robots.alohamini.lekiwi_uni_client import LeKiwiUniClient
...
def main():
    parser = argparse.ArgumentParser(description="Record episodes with uni-arm teleoperation")
    ...
    parser.add_argument("--leader_port", type=str, default="/dev/am_arm_leader_left", help="Leader arm device port")
    parser.add_argument("--leader_id", type=str, default="so100_leader_uni", help="Leader arm device ID")
    parser.add_argument("--display-data", action="store_true", help="Display data with rerun")
    ...
    robot_config = LeKiwiUniClientConfig(remote_ip=args.remote_ip, id=args.robot_id)
    leader_arm_config = UniSO100LeaderConfig(
        port=args.leader_port,
        id=args.leader_id,
    )
    keyboard_config = KeyboardTeleopConfig()

    robot = LeKiwiUniClient(robot_config)
